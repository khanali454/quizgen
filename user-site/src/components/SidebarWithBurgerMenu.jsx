    ...
    <>
      ...
      <div className="fixed top-0 left-0 w-full flex items-center justify-between px-4 py-2 bg-gray-100 shadow-md z-50">
        ...
        <Menu as="div" className="relative inline-block text-left">
          ...
          <Menu.Items className="absolute right-0 mt-2 w-48 origin-top-right bg-white rounded-lg shadow-lg focus:outline-none">
            <Menu.Item>
              {({ active }) => (
                <Link to="/profile" className={`flex items-center gap-2 px-4 py-2 text-sm ${active ? "bg-gray-100" : ""}`}>
            ...
            <Menu.Item>
              {({ active }) => (
                <Link to="/settings" className={`flex items-center gap-2 px-4 py-2 text-sm ${active ? "bg-gray-100" : ""}`}>
      ...
      >
        <div className="flex h-[calc(100vh-90px)] flex-col justify-between space-y-4">
          <div>
            <Link to="create-quiz" className="flex items-center justify-center bg-gradient-to-r from-blue-500 to-purple-500 rounded-lg text-white gap-3 p-3 text-normal font-normal">
              Generate New Quiz
            </Link>

            {/* ✅ Sidebar Links with Active State */}
            <Link to="/User-Dashbord" className={`flex items-center gap-3 p-3 text-lg font-normal rounded-lg 
  ${location.pathname === "/User-Dashbord" ? "text-blue-500 font-semibold" : "text-gray-700"}`}>
  <HomeIcon className="h-6 w-6" /> Dashboard
</Link>

<Link to="genrated-mcqs" className={`flex items-center gap-3 p-3 text-lg font-normal rounded-lg 
...
<Link to="all-files" className={`flex items-center gap-3 p-3 text-lg font-normal rounded-lg 
...
<Link to="settings" className={`flex items-center gap-3 p-3 text-lg font-normal rounded-lg 
